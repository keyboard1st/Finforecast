...
class CY312_TimeSeriesDataset(Dataset):
    def __init__(self, sample_set = 'inner',window_size=5, train_data_ref=None):
        # [B,T,C]
        if sample_set in ['inner', 'outer']:
            self.data = np.concatenate([load_x_CY312_parquet('CY_312_factor'), load_y_CY312_parquet(sample_set)], axis=-1)
        # 若为测试集且提供训练集引用
...
def get_CY312_TimeSeriesDataloader(batchsize = 1, shuffle_time = True, window_size = 30, num_val_windows = 100, val_sample_mode = 'random'):
    '''
    ...
    # 拼接训练集和测试集
    daydata_train = CY312_TimeSeriesDataset('inner', window_size)
    train_last_window = daydata_train.data[:, -window_size+1:, :]
    daydata_test_original = CY312_TimeSeriesDataset('outer', window_size)
    augmented_test_data = np.concatenate(
        [train_last_window, daydata_test_original.data],
        axis=1
    )
    daydata_test = CY312_TimeSeriesDataset(sample_set='augmented_test',window_size=window_size,train_data_ref=augmented_test_data)
